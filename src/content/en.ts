import type { I18nDict } from '../i18n/types';

export const en: I18nDict = {
  header: {
    cave: 'Cave',
    manifesto: 'Manifesto',
    ancient: 'Ancient',
    exit: 'Exit',
    team: 'Creators',
    events: 'Events',
    natureUrban: 'Nature→Urban',
    gallery: 'Gallery',
    contact: 'Contact',
    brandHint: 'Art of Creation',
  },
  cave: {
    title: 'Lingua Universalis',
    subtitle: 'The Art of Creation',
    manifestoTitle: 'Manifesto',
    manifesto: [
      'The language of art is the most ancient, universal, and understandable form of communication on Earth.',
      'Art is an instrument for exploring the human condition and our relationship with the Universe.',
      'In an age of fragmentation, the Art of Creation becomes a foundation for mutual understanding—a bridge between cultures.',
      'The aesthetic is a bridge between the prehistoric and the meta-modern: ancient and futuristic at once.',
      'Truth is hidden in the dark: knowledge reveals itself only when the viewer brings focus—the Light.',
    ],
    flashlightTitle: 'Flashlight',
    flashlightHint: 'Move the cursor to reveal the cave.',
    flashlightHint2: 'Scroll down—the journey begins.',
  },
  ancient: { title: 'Ancient marks' },
  exitFlight: {
    title: 'Exit',
    hint: 'Scroll to pass through the opening.',
  },
  team: { title: 'Creators (constellation)', hint: 'Hover/focus to pause and read.', pause: 'Pause' },
  events: { title: 'Events', hint: 'Hover/focus to pause. Scroll to continue.', pause: 'Pause', explore: 'Explore', scrollDiscover: 'Scroll to Discover' },
  gallery: {
    title: 'Works Gallery',
    lede: 'Search, filter by artist, and open a work. The URL becomes shareable for that work.',
    searchPlaceholder: 'Search by artist / title…',
    artistFilterAll: 'All artists',
    sortArtist: 'Artist A→Z',
    sortTitle: 'Title A→Z',
    lightboxPrev: 'Prev',
    lightboxNext: 'Next',
    lightboxClose: 'Close',
  },
  natureUrban: {
    title: 'Nature → Urban',
    lede: 'A short transition video will live here later. For now we use a pinned visual beat.',
  },
  contact: {
    title: 'Signal',
    hint: 'Press and hold to reveal contact channels.',
    revealedHint: 'Channels revealed. Choose a path.',
    emailLabel: 'Email',
    telegramLabel: 'Telegram',
    open: 'Open',
    mail: 'Mail',
    copy: 'Copy',
    copied: 'Copied',
  },
};


